...
use super::*;
use super::engine::{
    SeriesTemplate, TOURNAMENT_MIN_DISPLACEMENTS, leaf_steady_adopt_into, leaf_steady_plan,
    leaf_steady_plan_cached,
};
...
// ------------------------------------------------------------------
// leaf_steady_cache: shared leaf plan/list snapshot adversarial tests
...
fn leaf_plan_cache_adopted_state_equals_fresh_derivation() {
    ...
    let snap = leaf_steady_cache::get(template.0.feasible_decompositions_handle().table_id(), &b)
...
fn leaf_plan_cache_snapshot_immune_to_fold_mutation() {
    ...
    let before = leaf_steady_cache::get(table_id, &b1).expect("snapshot present");
    ...
    let after = leaf_steady_cache::get(table_id, &b1).expect("snapshot survives");
    assert_eq!(

...
#grid(columns: (70pt, 1fr, 105pt), column-gutter: 7pt, row-gutter: 4pt,
  ...
  [An associative reduction of the per-segment displacements by BCH concatenation: $S = (((d_1 ⊕ d_2) ⊕ d_3) ⊕ dots.c)$. Computed as an *adaptive balanced tournament over chunks* — associativity lets the reduction tree take any shape, and the driver exploits it (§8); the strictly sequential chain remains the single-threaded and small-batch path.],
  align(horizon, cap(7pt)[_a data dependency across folds; associativity is the one escape hatch, taken (§8)_]),
...
#text(size: 8.2pt)[
  #grid(columns: (66pt, 1fr, 72pt), column-gutter: 6pt, row-gutter: 3.5pt,
    ...
    align(left)[*④ Sweep*], align(left)[Workers claim packs dynamically. Per ticket: compute the antisymmetric term (1–2 multiplies) and add its decomposition-row contributions straight into the result buffer — one phase, direct-add, no staging.], align(horizon, cap(6.6pt)[the *only* heavy parallel region]),
  )
]
...
#text(size: 8.4pt)[
  #grid(columns: (96pt, 1fr), column-gutter: 6pt, row-gutter: 3pt,
    align(left)[*Nodes*], align(left)[Bracket subterms, *interned* so shared subtrees (the same nested bracket can appear in many BCH terms) are computed once. Each node is one commutation kernel call — and, by content homogeneity, each node writes *exactly one anagram class* of target words, swept as that class's per-unit bundles.],
...
#cap(6.8pt)[*Update, 2026-09-05 (current head).* The table above is kept as the campaign's record. Since it was measured, the kernel's scheduling cell became the anagram unit with a single-phase direct-add sweep (§3.3), per-build tables and derived plans became process-wide caches, and cohort engagement became purely capability-based. Fresh 1000-segment e2e medians on the same machine, current head: 8×3 *1.09 ms* \@32t (was 2.08 in the table — the last gate removal), 2×12 *216 ms* \@32t, 3×8 *48 ms* \@32t, 4×6 *20.0 ms* \@4t, 2×8 *3.99 ms* \@32t. The regime shape is unchanged: low-degree grids stay dispatch-bound and high-degree grids stay bandwidth-bound; the constants moved.]
